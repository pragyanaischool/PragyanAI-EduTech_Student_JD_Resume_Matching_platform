...
tab_cv, tab_match, tab_ats, tab_cover, tab_mock, tab_roadmap = st.tabs([
    "📝 1. Resume Builder & 3-Box Copilot",
    "🎯 2. JD Match & SWOT",
...
# ==============================================================================
# TAB 1: RESUME BUILDER, 3-BOX COPILOT & MODIFIED MASTER HUB
# ==============================================================================
with tab_cv:
    st.header("Step 1: Resume Ingestion, 3-Box Copilot & Full Modified Resume Hub")
    ...
    with col_preview:
        st.subheader("2. Master Resume Status")
        if st.session_state.cand_cv:
            st.success("✅ Master Resume is loaded and ready for section polishing.")
            st.markdown(f"**Current Sections Extracted:** `{len([k for k, v in st.session_state.sec_current.items() if v.strip()])}` | **Active Revision:** `#{st.session_state.master_rev}`")
            st.info("💡 Review and polish individual sections in the 3-Box Copilot below. Applying modifications automatically syncs your Master Markdown and live PDF document.")
        else:
            st.info("Ingest your resume on the left to activate 3-Box Copilot, master live editor, and PDF export.")

    st.markdown("---")

    # ==========================================================================
    # 3-BOX SECTION POLISH COPILOT (CURRENT -> AI REFINED -> MODIFIED & APPLY)
    # ==========================================================================
    if st.session_state.cand_cv:
        st.subheader("🛠️ 3-Box Section Polish Copilot")
        st.caption("Auto-extracted sections from your master resume. Compare current text, inspect AI refinement with Google X-Y-Z formula, tweak in the Modified box, and apply directly to update the Full Resume below.")
        ...
        for tab_ui, sec_name, btn_label in sections_config:
            with tab_ui:
                ...
                # Dynamic polish revision index for key freshness
                ...
                with c1:
                    ...
                    edited_curr = st.text_area(
                        ...
                        value=curr_val,
                        height=220,
                        key=f"box1_curr_{sec_name}_{m_rev}_{p_rev}",
                        placeholder=f"Enter or edit existing {sec_name} text..."
                    )
                    st.session_state.sec_current[sec_name] = edited_curr
                    
                    if st.button(f"✨ {btn_label}", key=f"btn_polish_{sec_name}_{m_rev}_{p_rev}", type="primary", use_container_width=True):
                ...
                with c2:
                    ...
                    st.text_area(
                        ...
                        value=ai_val,
                        height=220,
                        key=f"box2_ai_{sec_name}_{m_rev}_{p_rev}",
                        placeholder="Click the polish button on the left to generate an AI-enhanced version..."
                    )
                    if ai_val and st.button(f"Copy to Modified ➡️", key=f"btn_copy_{sec_name}_{m_rev}_{p_rev}", use_container_width=True):
                ...
                with c3:
                    ...
                    edited_mod = st.text_area(
                        ...
                        value=mod_val,
                        height=220,
                        key=f"box3_mod_{sec_name}_{m_rev}_{p_rev}",
                        placeholder="Customize final text here before applying to master..."
                    )
                    st.session_state.sec_mod[sec_name] = edited_mod

                    if st.button(f"💾 Apply Modified {sec_name} to Master", key=f"btn_apply_{sec_name}_{m_rev}_{p_rev}", type="primary", use_container_width=True):
                        if edited_mod.strip():
                            ...
                            st.success(f"Merged updated {sec_name} into Master Resume & Database!")
                            st.rerun()
                        else:
                            st.warning("Modified section cannot be empty.")

        # ======================================================================
        # FULL MODIFIED RESUME PREVIEW & DUAL EXPORT (PDF + MD)
        # ======================================================================
        st.markdown("---")
        st.subheader("📄 Full Modified Resume Preview & Dual Export Hub")
        st.caption("Inspect your complete merged resume in raw Markdown or rendered presentation format, and download as publication-ready ATS PDF or Markdown (.md).")

        col_full_md, col_full_view = st.columns([1, 1])

        with col_full_md:
            st.markdown("#### 📝 Master Markdown Source Code")
            master_md_val = st.text_area(
                "Live Editable Master Markdown Document",
                value=st.session_state.cand_cv,
                height=380,
                key=f"master_resume_full_editor_{m_rev}"
            )
            if master_md_val != st.session_state.cand_cv:
                st.session_state.cand_cv = master_md_val

            col_btn_md, col_btn_save = st.columns(2)
            with col_btn_md:
                st.download_button(
                    label="📥 Download Resume (.MD)",
                    data=st.session_state.cand_cv.encode("utf-8"),
                    file_name="Modified_ATS_Resume.md",
                    mime="text/markdown",
                    use_container_width=True
                )
            with col_btn_save:
                if st.button("💾 Save Raw MD Edits to DB", key="save_raw_md_btn", use_container_width=True):
                    sql_db.save_resume(
                        filename="Updated_Resume.md",
                        raw_content=st.session_state.cand_cv,
                        markdown_content=st.session_state.cand_cv,
                        user_id=current_user.get("id")
                    )
                    st.success("Master resume saved to Database!")

        with col_full_view:
            st.markdown("#### 👁️ Rendered Document & PDF Export")
            ...
            st.download_button(
                ...
                data=pdf_bytes,
                file_name="Modified_ATS_Resume.pdf",
