...
with tab_approvals:
    ...
    if pending_users:
        pending_data = [{
            ...
            "Role": u.role.upper(),
            "Registered At": u.created_at.strftime("%Y-%m-%d %H:%M")
        ...
        with col_app1:
            selected_user_id = st.selectbox("Select Account to Moderate", [p["User ID"] for p in pending_data], format_func=lambda x: f"User ID #{x} - {next(p['Email'] for p in pending_data if p['User ID'] == x)}")
